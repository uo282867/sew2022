...
class CalculadoraEspecializada extends CalculadoraRPN {
    ...
    realizarAccion(tecla) {
        switch(tecla) {
            case 'm': this.media(); break;
            case 'v': this.varianza(); break;
            case 'd': this.desvTipica(); break;
            case 'f': this.coefVariacion(); break;

            default: super.realizarAccion(tecla);
        }
    }

    media() {
        var sumando = 0;
        var tam = this.pila.length;
        
        while (this.pila.length > 0) {
            sumando += this.pila.pop();
        }

        this.pila.push(new Number(sumando/tam));
        this.mostrarDatos();        
    }
    ...
    varianza() {
        ...
        while (this.pila.length > 0) {
            sumando += Math.pow(new Number(this.pila.pop() - media),2);
        }
    ...
    desvTipica() {
        ...
        this.pila.push(new Number(Math.sqrt(new Number(sumando/tam))));
        this.mostrarDatos();  
    }
    ...
    coefVariacion(){
        ...
        this.pila.push(new Number(Math.sqrt(new Number(sumando/tam))/Math.abs(media)));
        this.mostrarDatos();  
    }
